...
const generateBlocks = (data: any, color: any) => {
  let output = ``;
  for (let j = 0; j < data.blocks.length; j++) {
    const col = color.legend[`color-${j + 1}`];
    const noCol = col.slice(1);
    const classe = `sezione-${data.title.replace(/ /g, '_')}-${data.blocks[j]}-${noCol}`;
...
const generateSections = (data: any) => {
  let output = ``;
...
const generateHtml = (data: any, css: any) => {
  const html = `
  <head>
  <style>
    ${css}
